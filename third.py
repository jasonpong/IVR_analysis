...
def analyze_directory(directory_path, output_csv='results.csv'):
    ...
    for i, wav_file in enumerate(wav_files, 1):
        ...
        try:
            ...
            results.append({
                'filename': os.path.basename(wav_file),
                'ticks_found': 0, 'cluster_span': 0, 'tick_density': 0, 'intervals': [],
                'mean_interval': 0, 'interval_cv': 0, 'amplitude_cv': 0,
                'mean_duration': 0, 'duration_cv': 0, 'assessment': 'ERROR', 'error': str(e)
            })
